...
class IrDefault(models.Model):
    ...
    @api.model
    def set_hsg_favicon(self, model, field):
        script_dir = os.path.dirname(__file__)
        rel_path = "../static/src/img/favicon.png"
        abs_file_path = os.path.join(script_dir, rel_path)
        with open(abs_file_path, "rb") as image_file:
            encoded_string = base64.b64encode(image_file.read())
            self.set('res.config.settings', 'hsg_favicon', encoded_string.decode("utf-8"))

class ResConfigSettings(models.TransientModel):
    _inherit = 'res.config.settings'

    hsg_favicon = fields.Binary(string="Favicon Image")
    ...
    def get_debranding_settings(self):
        ...
        hsg_favicon = IrDefault._get('res.config.settings', "hsg_favicon")
    ...
    def set_values(self):
        ...
        IrDefault = self.env['ir.default'].sudo()
        IrDefault.set('res.config.settings', "hsg_favicon", self.hsg_favicon.decode("utf-8"))
        ...
        if not self.attach_id:
            ...
            if attach_id:
                attach_id.write({
                    'datas': self.hsg_favicon.decode("utf-8"),
                })
            else:
